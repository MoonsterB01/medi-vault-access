-- Drop the problematic security definer functions that lose auth context
DROP FUNCTION IF EXISTS public.user_can_access_patient(uuid);
DROP FUNCTION IF EXISTS public.user_hospital_can_access_patient(uuid);

-- Drop all existing policies to start fresh
DROP POLICY IF EXISTS "Admins have full access" ON public.family_access;
DROP POLICY IF EXISTS "Users can view their own access records" ON public.family_access;
DROP POLICY IF EXISTS "Hospital staff can manage access for their patients" ON public.family_access;
DROP POLICY IF EXISTS "Users can view documents they have access to" ON public.documents;
DROP POLICY IF EXISTS "Hospital staff can upload documents for their patients" ON public.documents;
